...
B<qev is not a drop-in replacement for libev, and does not try to be.> Every C
symbol carries a C<qev_> prefix -- C<ev_run>, C<ev_io_start>, C<ev_timer_init> --
so an installed qev and an installed libev can sit in one prefix, and one
translation unit can include both headers, without either shadowing the other.
Migrating from libev is a mechanical C<ev_> to C<qev_> rename and nothing else;
the semantics of every call below are libev's. The C<EV_*> B<macros> keep their
upstream spelling on purpose: a differing redefinition is a required diagnostic in
C, so that half of the namespace reports itself rather than colliding in silence.
C<ev_version_major ()> reports C<5>.

The manual below is libev's, carried across the same rename the code took. The
points that follow summarise what differs from upstream libev 4.x.
