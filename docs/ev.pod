...
qev - a high performance full-featured event loop written in C

=head1 QEV (VERSION 5)

This manual documents B<qev>, a modernized, cross-platform fork of libev
(L<http://software.schmorp.de/pkg/libev.html>) maintained as the event-loop core of
the qb Actor Framework and published standalone. It derives from Marc Alexander
Lehmann's libev under BSD-2; see F<THIRD-PARTY-NOTICES> for the per-file map.

B<The C API is libev's, unchanged.> C<ev_run>, C<ev_io_start>, C<ev_timer_init>,
C<struct ev_loop> -- every name, every signature, every semantic. Porting a libev
program is one include-path edit and nothing else.

B<What differs is the header path.> qev installs under F<include/qev/>, so the
include is C<< #include <qev/ev.h> >>. The manual below is upstream libev's and
spells it C<< #include <ev.h> >> throughout; substitute the qev path when copying
from it. Everything reaching a filesystem is likewise qev's own -- the archive is
F<libqev.a>, the CMake package is C<find_package(qev)>, pkg-config is F<qev.pc> --
so an installed qev never overwrites an installed libev, and one translation unit
can include C<< <qev/ev.h> >> and a real C<< <ev.h> >> together.

B<What you cannot do is link both into one program.> Both export the same C<ev_*>
symbols and libev is a single translation unit, so an archive is all-or-nothing:
the linker either refuses with a duplicate-symbol error, or -- when the program
touches only the symbols both provide -- resolves them silently from whichever
archive came first. Pick one.

qev is B<not> binary-compatible with libev 4.x and does not claim to be:
C<ev_version_major ()> reports C<5>, and libev's own documented check,
C<assert (ev_version_major () == EV_VERSION_MAJOR)>, a 4.x consumer fails by
construction. Recompile; do not swap the object.

The manual below is libev's, unchanged. The points that follow summarise what
differs from upstream libev 4.x.

Platform defaults and backends:

=over 4

=item *

macOS and the BSDs now default to C<kqueue> instead of C<select> (which was capped
at C<FD_SETSIZE> and O(N) per poll).

=item *

Windows gains a hardened C<epoll> backend implemented with wepoll (IOCP): a
correct file-descriptor-to-C<SOCKET> mapping, C<EPOLLRDHUP> TCP half-close, and a
one-loop-per-thread model. C<select> is no longer the only Windows option.

=item *

C<select> is demoted to a strict last-resort fallback on every platform.

=item *

The C<io_uring> backend was rewritten for correctness: ring-layout validation,
C<mmap> fallback with bounds/overflow checks, correct SQ/CQ memory fences,
generation-counter stale-completion filtering, and CQ-overflow recovery. It is
compiled when available but, like C<linuxaio>, is not auto-selected.

=back

Correctness, robustness and portability fixes: C<array_nextsize> grows in
C<size_t> with an C<INT_MAX> clamp; the C<epoll> backend range-checks the fd
before indexing; C<linuxaio> bounds its C<io_cancel> retry loop; the inotify read
buffer is properly aligned; C<ev_win32> uses C<INVALID_SOCKET> sentinels;
strict-aliasing-safe C<offsetof> accessors and a compile-time ABI contract were
added (no ABI change); the libevent compatibility layer is self-contained and
C<event_pending> reports true expiry.

The library builds warning-clean under a strict GCC/Clang flag set and passes
ASan/UBSan. It is distributed under the MIT license; portions derived from libev
and wepoll remain under their original BSD-2-Clause terms (see THIRD-PARTY-NOTICES).

The original libev manual follows unchanged.
...
time: L<http://pod.tst.eu/http://cvs.schmorp.de/libev/qev.pod>.
...
C<SIGCHLD> signal handler I<after> calling C<ev_default_loop>.
...
qev asks the kernel for a blocking wait in nanoseconds where it can:
C<epoll_wait> takes whole milliseconds and libev rounds up, so a timeout
under a millisecond -- a timer 200 us away, an embedder parking its
thread for 100 us -- used to sleep a full one. On Linux 5.11 or newer
the wait goes through C<epoll_pwait2> and is honoured to the thread's
timer slack (50 us by default, C<prctl (PR_SET_TIMERSLACK)>); the kernel
is asked once at loop initialisation and an older one keeps
C<epoll_wait>. See C<EV_USE_EPOLL_PWAIT2>.
...
=item ev_now_set (loop, ev_tstamp mono)

qev. The embedder hands the loop a reading of C<ev_clock_now ()> it already
made -- qb's core reads the monotonic clock on its idle passes for its own
park policy -- and the next C<EVRUN_NOWAIT> pass reads no clock of its own:
its tail time update, the one read a non-blocking pass still makes, stands
down for that ONE pass. Timers are judged against the supplied time, so a
sample ahead of the wall clock fires a timer at once and a sample behind it
delays one; a sample older than the loop's time is ignored, the loop's clock
never stepping back; a pass that blocks always re-reads, since it slept. The
realtime clock (C<ev_now>) follows the sample the way the loop's own update
derives it. Call it right before the pass it is meant for: a supply that
outlives its pass makes a later pass judge timers against an old time.
Without a monotonic clock there is nothing to supply, and the call is a
plain C<ev_now_update>.

=item ev_tstamp ev_clock_now ()

qev. The monotonic clock the loop's timers run on -- C<CLOCK_MONOTONIC>,
C<QueryPerformanceCounter> on Windows -- as C<ev_run> itself reads it: the
sample C<ev_now_set> takes, and the scale C<ev_timer_next> answers on.
Before the first loop exists the monotonic probe has not run and this reads
the realtime clock, as C<ev_run> would then.
...
C<EVRUN_NOPOLL> (qev), combined with C<EVRUN_NOWAIT>, runs the iteration
without calling the backend at all: timers and periodics are reified and
every pending event is invoked exactly as without it, only the readiness
poll is left out. It exists for an embedder that drives the loop from its
own scheduler on every pass of that scheduler and wants to decide itself
how often a quiet file descriptor is looked at (the poll is a system call
on every pass, and on a quiet descriptor it returns nothing on nearly all
of them). A pass that would block ignores the flag: with a descriptor to
wait on, the poll is the wake. C<ev_io_fed_addr> tells such an embedder
whether the last poll found anything.
...
=item int ev_active_count (loop)

Returns the number of referenced active watchers - the count that keeps
C<ev_run> looping (an C<ev_unref>'d watcher is not counted). Together with
C<ev_pending_count> it lets an embedder that drives the loop with
C<EVRUN_NOWAIT> from its own scheduler skip a run that could do nothing:
when both are zero there is no watcher to fire and no event queued, so the
backend poll would only cost its syscall.

=item unsigned int ev_io_count (loop)

Returns the number of active C<ev_io> watchers, i.e. what the backend
poll has to look at. The loop's own internal watchers on real file
descriptors count (the timerfd behind periodics, the signalfd); the pipe
behind signal and async watchers does not, because what it carries reaches
the loop through flags a non-blocking pass reads directly. When this is
zero, an C<ev_run> that would not block anyway (C<EVRUN_NOWAIT>, or an
expired timer) skips the backend poll altogether; a blocking run still
polls, because with no fd the poll is the sleep.

=item const int *ev_io_count_addr (loop)

=item const unsigned int *ev_io_fed_addr (loop)

The address of the count above, and of the number of ready file
descriptors the backend has reported since the loop was initialised (one
per descriptor with pending events after a poll, whatever the number of
watchers on it; it only grows, and wraps). Both read inline, valid until
C<ev_loop_destroy>, for the embedder that consults them on every pass: the
first says whether a poll would look at anything, the second, compared
across passes, whether the last poll found something -- the two inputs of
a poll cadence that stays at one poll per pass while a descriptor is busy
and backs off while it is quiet.

=item const int *ev_timer_count_addr (loop)

=item ev_tstamp ev_timer_next (loop)

=item const EV_ATOMIC_T *ev_wake_pending_addr (loop)

qev. The three remaining inputs of an embedder that decides, per pass,
whether the loop has anything to do at all -- and does not call C<ev_run>
when it has not. The first is the number of active C<ev_timer> watchers,
read inline. The second is the earliest deadline the loop holds, on the
clock C<ev_clock_now ()> reads: the top of the timer heap, and of the
periodic heap brought onto the monotonic scale when periodics are compiled
in; a value no reading can reach (C<1e13> seconds or more) when it holds
none. C<timers_reify> fires a timer once the loop's time has passed its
deadline, so a deadline at or before the embedder's reading is due, one
after it is not. The third is the address of the flag C<ev_async_send>
(and a signal) raises from any thread while no pass wanted the wake pipe
written -- the loop was not blocking -- cleared by the pass that delivers
it: an embedder that skips passes runs one while it is set. With
C<ev_pending_count_addr> (an event fed since the last pass) and
C<ev_io_count_addr> (a poll to make), that is every reason a non-blocking
pass could have to enter the loop; qb 3.2's core pays a few loads for a
pass with none, where it paid the loop's clock read and bookkeeping.
...
used).
   ...
   struct ev_loop *loop_hi = ev_default_loop (0);
...
execution of C<EV_DEFAULT>, C<EV_DEFAULT_> or C<ev_default_loop (...)>.
...
whatever way you want, you can also C<m4_include([qev.m4])> in your
...
=item EV_USE_EPOLL_PWAIT2

If defined to be C<1>, the epoll backend makes its blocking waits through
C<epoll_pwait2>(2), which takes a C<struct timespec>, so a timeout under
a millisecond is honoured to the thread's timer slack instead of being
rounded up to a whole millisecond; kernel support (Linux 5.11) is probed
once per loop and an older kernel falls back to C<epoll_wait>. If
undefined, it will be enabled if the headers indicate GNU/Linux + Glibc
2.35 or newer (the first libc to declare the function), otherwise
disabled. Requires C<EV_USE_EPOLL>.
...
the documentation for C<ev_default_loop>.
