...
libev - a high performance full-featured event loop written in C (qb-ev fork)

=head1 QB-EV FORK (VERSION 5)

This manual documents B<qb-ev>, a modernized, cross-platform fork of libev
(L<http://software.schmorp.de/pkg/libev.html>) maintained as the event-loop core
of the qb Actor Framework. The C<ev_*> API and ABI are unchanged, so the entire
manual below applies verbatim; C<ev_version_major ()> now reports C<5>. The points
below summarise what differs from upstream libev 4.x.
