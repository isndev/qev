...
/*
 * Single-loop-per-thread execution model (what qb-ev is designed for):
 * - One struct ev_loop per OS thread; all ev_* calls and watcher callbacks for
 *   that loop run on that thread only.
 ...
 *   unsafe. One loop per thread matches wepoll's intended use.
